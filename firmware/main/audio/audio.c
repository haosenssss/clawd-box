...
/*
 * 等你输入：**G5→E5 下行小三度，原地重复三遍。**
 *
 * 和完成音必须在多个维度上同时对立，只改音高不够——
 * 之前两个都是"上行 + 高音区 + 和弦收尾"，听感几乎一样：
 *
 *              完成音               等待音
 *   走向       逐级上行跑动          原地重复，不移动
 *   音程       二度/三度上行         下行小三度（"叮—咚"，喊人那个动机）
 *   收尾       高位大三和弦，解决     停在低音，不解决
 *   音区顶点   C6 (1047Hz)          G5 (784Hz)
 *
 * 下行小三度是通用的"叫人"动机（门铃、隔着院子喊名字），
 * 不解决的收尾正对应"还没完，等着你"。重复三遍是因为这是催促
 * 不是通知——重复本身就是"需要你动手"的信号。
 */
static const note_t SEQ_NEEDS[] = {
    {783.99f, 150}, {659.25f, 210}, {0.0f, 110},
    {783.99f, 150}, {659.25f, 210}, {0.0f, 110},
    {783.99f, 140}, {659.25f, 340},
    {0, 0}};
